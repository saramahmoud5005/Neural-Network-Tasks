...
import pengunis_classification
...
def getValues():
    ...
    dictionary['LearningRate'] = int(learningRate.get())
    dictionary['Epochs'] = int(epochs.get())
...
def Train():
    print("Train")
    getValues()
    W, x_train, y_train, x_test, y_test = pengunis_classification.perceptron_learing(dictionary['LearningRate'],
                                                                                     dictionary['Epochs'],
                                                                                     dictionary['Bias'],
                                                                                     dictionary['Feature1'],
                                                                                     dictionary['Feature2'],
    ...
    res.append(x_test)
    res.append(y_test)
    acc = pengunis_classification.evaluate(res[1], res[2], res[0])
    accuracy_train(acc)


def Test():
    print("Test")
    acc = pengunis_classification.evaluate(res[3], res[4], res[0])
    accuracy_test(acc)
...
def visualize():
    print("visualize")
    getValues()
    dataset = pengunis_classification.load_data()
    df = dataset.copy()
    pengunis_classification.visualize(dictionary['Feature1'], dictionary['Feature2'], df)


def accuracy_test(acc):
    dictionary['Accuracy_test'] = acc
    label_Accuracy1 = Label(App, text="Accuracy Testing = " + str(dictionary['Accuracy_test']), font=25)
    label_Accuracy1.grid(row=16, column=1, padx=5, pady=5)


def accuracy_train(acc):
    dictionary['Accuracy_train'] = acc
    label_Accuracy2 = Label(App, text="Accuracy Train = " + str(dictionary['Accuracy_train']), font=25)
...
label_bias.grid(row=7, column=1, padx=5, pady=5)

Radiobutton_bias = Radiobutton(App, text="YES", variable=bias, value=True)
Radiobutton_bias.grid(row=8, column=1, padx=2)
